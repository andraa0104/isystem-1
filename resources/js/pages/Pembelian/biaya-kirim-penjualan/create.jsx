...
export default function BiayaKirimPembelianCreate() {
    ...
    const [fieldBiayaKirim, setFieldBiayaKirim] = useState('');
    ...
    const [fieldAddBiayaKirim, setFieldAddBiayaKirim] = useState('');
    ...
    const shippingSalesPercent = useMemo(() => {
        if (totalModal === 0) return 0;
        const value = ((totalSales - totalModal) / totalModal) * 100;
        return Number(value.toFixed(2));
    }, [totalSales, totalModal]);

    const finalMarginPercent = useMemo(() => {
        const cost = toNumber(biayaKirim);
        const profit = totalSales - totalModal - cost;
        if (totalModal === 0) return 0;
        const value = (profit / totalModal) * 100;
        return Number(value.toFixed(2));
    ...
    const handleSelectMaterial = async (row) => {
        ...
        if (refPo) {
            ...
            const biayaRes = await fetch(
                `/penjualan/biaya-kirim-penjualan/biaya-kirim?ref_po=${encodeURIComponent(refPo)}&material=${encodeURIComponent(materialValue)}`,
                { headers: { Accept: 'application/json' } }
            );
    ...
    const handleSelectMaterialAdd = (row) => {
        ...
        if (refPo) {
            fetch(
                `/penjualan/biaya-kirim-penjualan/biaya-kirim?ref_po=${encodeURIComponent(refPo)}&material=${encodeURIComponent(row.mat ?? row.material ?? '')}`,
                { headers: { Accept: 'application/json' } }
            )
                .then((res) => res.json())
                .then((data) => {
    ...
    useEffect(() => {
        if (!fieldAddPriceSell) return;
        const qty = toNumber(fieldAddQty);
        const sellPrice = toNumber(fieldAddPriceSell);
        const totalSell = qty * sellPrice;
    ...
    const handleAddDoAddRow = () => {
        ...
        setDoAddRowsTable((prev) => [
            ...prev,
            {
                ...
                biaya_kirim: fieldAddBiayaKirim,
            },
        ]);
        setFieldAddKdMat('');
        setFieldAddMat('');
