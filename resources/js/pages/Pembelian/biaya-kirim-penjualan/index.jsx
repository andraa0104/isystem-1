...
export default function BiayaKirimPenjualanIndex({
    items = [],
    summary = {},
    filters = {},
}) {
    ...
    const [dotPageSize, setDotPageSize] = useState(5);
    const [materialCurrentPage, setMaterialCurrentPage] = useState(1);
    const [dotCurrentPage, setDotCurrentPage] = useState(1);
    ...
    const fetchItems = async () => {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error('Gagal memuat data biaya kirim penjualan.');
            const data = await res.json();
            setRemoteItems(Array.isArray(data?.items) ? data.items : []);
            setRemoteSummary(
                data?.summary ?? { unpaid_count: 0, unpaid_total: 0 },
            );
        } catch (err) {
    ...
    const filteredDotRows = useMemo(() => {
        const term = dotSearch.trim().toLowerCase();
        if (!term) return dotRows;
        return dotRows.filter((row) =>
            [row.kd_mat, row.mat]
                .map((val) => String(val ?? '').toLowerCase())
                .some((val) => val.includes(term)),
        );
    ...
    const dotTotalPages = useMemo(() => {
        if (dotPageSize === Infinity) return 1;
        const size = dotPageSize || 5;
        return Math.max(1, Math.ceil(filteredDotRows.length / size));
    }, [filteredDotRows.length, dotPageSize]);
    ...
    const displayedDotRows = useMemo(() => {
        if (dotPageSize === Infinity) return filteredDotRows;
        const size = dotPageSize || 5;
        const start = (dotCurrentPage - 1) * size;
        return filteredDotRows.slice(start, start + size);
    }, [filteredDotRows, dotPageSize, dotCurrentPage]);
    ...
    const dotGrandTotal = useMemo(() => {
        return filteredDotRows.reduce(
        ...
        try {
            ...
            if (row.no_dob && String(row.no_dob) !== '0') {
                ...
                if (dotRes.ok) {
                    ...
                    setDotRows(
                        Array.isArray(dotData?.rows) ? dotData.rows : [],
                    );
                }
            }
        } catch (err) {
                ...
                <DialogContent className="fixed inset-0 !top-0 !left-0 z-[210] !flex !h-screen !w-screen !max-w-none !translate-x-0 !translate-y-0 !flex-col overflow-hidden !rounded-none !p-0">
                    <div className="flex h-full w-full flex-col bg-[radial-gradient(1200px_600px_at_10%_-10%,rgba(255,255,255,0.06),transparent),radial-gradient(900px_500px_at_90%_-20%,rgba(255,255,255,0.04),transparent)]">
                        <div className="flex h-full min-h-0 w-full flex-col">
                            ...
                            <div className="h-full min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-6">
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-4 border-b bg-background/80 px-4 py-4 backdrop-blur sm:px-6">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        {viewHeader?.no_bkj
                                            ? `No BKJ: ${viewHeader.no_bkj}`
                                            : 'Memuat data...'}
                                    </p>
                                </div>
                            </div>

                            <div className="h-full min-h-0 flex-1 overflow-y-auto px-4 py-4 pb-32 sm:px-6">
                                {viewError ? (
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="flex items-center justify-between text-sm">
                                                    ...
                                                    <span className="text-lg font-bold">
                                                        {formatRupiah(
                                                            toNumber(
                                                                viewHeader?.totalbeli_biayajual,
                                                            ),
                                                        )}
                                                    </span>
                                                </div>
                                                <div className="flex items-center justify-between text-sm">
                                                    ...
                                                    <span className="font-semibold">
                                                        ...
                                                        toNumber(
                                                            viewHeader?.sisa_bayar ??
                                                                0,
                                                        ) === 0
                                                            ? formatRupiah(0)
                                                            : formatRupiah(
                                                                  viewHeader?.jumlah_bayar,
                                                              )}
                                                    </span>
                                                </div>
                                                <div className="flex items-center justify-between text-sm">
