...
export default function JurnalPenyesuaianPrint() {
    ...
    useEffect(() => {
        const run = async () => {
            ...
            try {
                ...
                params.set('sortBy', initialQuery?.sortBy ?? 'Periode');
    ...
    return (
        <div className="min-h-screen bg-white text-slate-900">
            <Head title="Jurnal Penyesuaian - Print" />
            <style>{`
            ...
            <div className="mx-auto max-w-[1100px] px-4 py-6">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-semibold">Jurnal Penyesuaian</h1>
                        <div className="mt-1 text-sm text-slate-600">
                            Periode: {getPeriodLabel(initialQuery?.periodType ?? 'month', initialQuery?.period ?? '')}
                        </div>
                        <div className="mt-1 text-sm text-slate-600">
                            Sumber: `tb_jurnalpenyesuaian`
                        </div>
                    </div>
                    <div className="text-right text-xs text-slate-600">
                        ...
                        <div className="mt-1">
                            ...
                            {(initialQuery?.sortBy ?? 'Periode') + ' ' + (initialQuery?.sortDir ?? 'desc')}
                        </div>
                        <div className="mt-1">Total dokumen: {new Intl.NumberFormat('id-ID').format(total)}</div>
                    </div>
                </div>

                <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
                    <div className="rounded-lg border border-slate-200 p-3">
                        <div className="text-[11px] uppercase tracking-wide text-slate-500">Total Dokumen</div>
                        <div className="mt-1 text-lg font-semibold">{new Intl.NumberFormat('id-ID').format(summary.total_dokumen)}</div>
                    </div>
                    <div className="rounded-lg border border-slate-200 p-3">
                ...
                <div className="relative mt-4 overflow-x-auto rounded-xl border border-slate-200">
                    ...
                    <div className="divide-y divide-slate-200">
                        {rows.map((j, idx) => {
                            ...
                            const periodeDoc = String(j?.Periode ?? '');
                            const details = Array.isArray(j?.details) ? j.details : [];
                            const has00 = Boolean(j?.has_00);

                            return (
                                <div key={`${kodeJurnal}|${periodeDoc}-${idx}`} className="p-4">
                                    <div className="flex flex-wrap items-start justify-between gap-3">
                                        <div>
                                            <div className="text-sm font-semibold text-slate-900">
                                                {formatDate(periodeDoc)} • {kodeJurnal}
                                            </div>
                                            <div className="mt-1 text-xs text-slate-600">
                                                Posting: {formatDate(j?.Posting_Date)} • Lines: {new Intl.NumberFormat('id-ID').format(j?.lines ?? 0)}
                                            </div>
                                            <div className="mt-1 text-xs text-slate-600">
                                                Remark: {j?.Remark || '-'}
                                            </div>
                                            ...
                                            <tbody>
                                                {details.length === 0 ? (
                                                    ...
                                                    details.map((d, di) => {
                                                        ...
                                                        return (
                                                            <tr key={`${kodeJurnal}|${periodeDoc}-${kodeAkun}-${di}`}>
