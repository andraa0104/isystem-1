...
export default function BiayaKirimPembelianCreate() {
    ...
    const fetchPoRows = async () => {
        ...
        try {
            ...
            const res = await fetch(
                `/pembayaran/biaya-kirim-pembelian/po-list?${params.toString()}`,
                {
    ...
    const handleSelectPo = async (row) => {
        ...
        const res = await fetch(
            `/pembayaran/biaya-kirim-pembelian/po-materials?no_po=${encodeURIComponent(row.no_po)}`,
            {
                headers: { Accept: 'application/json' },
            },
        );
    ...
    const handleSelectMaterial = async (row) => {
        ...
        if (refPo && row.kd_mat) {
            ...
            const margin =
                totalBuy === 0 ? 0 : ((totalSell - totalBuy) / totalBuy) * 100;
    ...
    return (
        <>
            <Head title="Tambah Biaya Kirim Pembelian" />
...
const breadcrumbs = [
    ...
    { title: 'Tambah', href: '/pembayaran/biaya-kirim-pembelian/create' },
];

BiayaKirimPembelianCreate.layout = (page) => (
