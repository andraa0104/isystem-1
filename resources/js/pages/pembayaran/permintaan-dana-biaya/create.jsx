...
export default function PermintaanDanaBiayaCreate() {
    ...
    const loadPayRows = async () => {
        ...
        try {
            ...
            params.set(
                'pageSize',
                payPageSize === 'all' ? 'all' : String(payPageSize),
            );
            const res = await fetch(
                `/pembayaran/permintaan-dana-biaya/paycost-rows?${params.toString()}`,
                {
    ...
    const addItem = async () => {
        ...
        if (!j || j <= 0) {
            await Swal.fire({
                icon: 'warning',
                title: 'Jumlah wajib lebih dari 0.',
            });
            return;
        }
        if (items.some((it) => String(it.kode_bayar).trim() === trimmedKode)) {
            await Swal.fire({
                icon: 'warning',
                title: 'Kode bayar sudah ada di daftar.',
            });
            return;
        }
    ...
    const selectPayCost = (row) => {
        ...
        const suggested =
            toNumber(row?.sum_sisa) > 0
                ? toNumber(row?.sum_sisa)
                : toNumber(row?.sum_total);
    ...
    return (
        <>
            <Head title="Tambah Permintaan Dana Biaya" />
            <div className="flex flex-col gap-6 p-6">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div className="space-y-1">
                        ...
                        <div className="text-sm text-muted-foreground">
                            Pilih Pay Cost → isi keterangan & jumlah → Masuk ke
                            daftar → Simpan.
                        </div>
                    </div>
                    ...
                    <Card className="xl:col-span-5">
                        ...
                        <CardContent className="space-y-5">
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/5 p-3">
                                <div className="text-xs text-muted-foreground">
                                    Tips: pilih Pay Cost dulu supaya Bayar &amp;
                                    Sisa ikut terisi.
                                </div>
                                <Button
                                    type="button"
                ...
                <DialogContent className="h-[calc(100vh-1.5rem)] w-[calc(100vw-1.5rem)] !max-w-[calc(100vw-1.5rem)] overflow-hidden p-0 sm:h-[calc(100vh-3rem)] sm:w-[calc(100vw-3rem)] sm:!max-w-6xl">
                    <DialogHeader>
                        <div className="border-b border-white/10 px-5 py-4 sm:px-6">
                            ...
                            <DialogDescription>
                                Data diambil dari tb_bayar (distinct
                                Kode_Bayar). Pilih salah satu untuk mengisi
                                form.
                            </DialogDescription>
                        </div>
                    </DialogHeader>

                    <div className="flex h-[calc(100%-76px)] flex-col gap-4 px-5 pt-4 pb-5 sm:h-[calc(100%-82px)] sm:px-6 sm:pb-6">
                        ...
                        <div className="flex flex-col gap-3 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div className="flex items-center justify-between gap-2 sm:justify-end">
                                <Button
                                    ...
                                    size="sm"
                                    disabled={
                                        payPage <= 1 || payPageSize === 'all'
                                    }
                                    onClick={() =>
                                        setPayPage((p) => Math.max(1, p - 1))
                                    }
                                >
                                    Sebelumnya
                                </Button>
...
PermintaanDanaBiayaCreate.layout = (page) => {
