...
export default function InvoiceMasukIndex({
    invoices = [],
    summary = {},
    filters = {},
}) {
    ...
    const fetchPaid = async (period, search = '') => {
        ...
        try {
            ...
            const res = await fetch(
                `/pembelian/invoice-masuk/paid?${params.toString()}`,
                {
                    headers: { Accept: 'application/json' },
                },
            );
            ...
            setPaidError(
                normalizeApiError(
                    err,
                    'Gagal memuat data invoice sudah dibayar.',
                ),
            );
            setPaidData([]);
    ...
    const unbilledTotalPages = useMemo(() => {
        ...
        return Math.max(
            1,
            Math.ceil(filteredUnbilled.length / unbilledPageSize),
        );
    ...
    const invoiceTotalPages = useMemo(() => {
        ...
        return Math.max(
            1,
            Math.ceil(filteredInvoiceItems.length / invoicePageSize),
        );
    ...
    const openPoDetail = async (refPo) => {
        ...
        try {
            ...
            if (
                !Array.isArray(data?.purchaseOrderDetails) ||
                data.purchaseOrderDetails.length === 0
            ) {
                setPoDetailError({
                    summary: 'Detail PO tidak ditemukan.',
                    detail: undefined,
                    status: 404,
                });
            }
    ...
    const openInvoiceDetail = async (noDoc) => {
        ...
        try {
            const res = await fetch(
                `/pembelian/invoice-masuk/${encodeURIComponent(noDoc)}`,
                {
                    headers: { Accept: 'application/json' },
                },
            );
            ...
            if (!data?.header) {
                setInvoiceError({
                    summary: 'Data invoice tidak ditemukan.',
                    detail: undefined,
                    status: 404,
                });
            }
    ...
    return (
        ...
        >
                        ...
                        <CardHeader className="pb-2">
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    ...
                                    <div className="text-xs text-muted-foreground">
                                        Berdasarkan tanggal bayar (tgl_bayar)
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    <Select
                                        value={paidPeriod}
                                        onValueChange={setPaidPeriod}
                                    >
                        ...
                        <CardContent className="grid gap-4 pt-2 sm:grid-cols-2">
                            ...
                            <div className="space-y-1 sm:text-right">
                                ...
                                <div className="text-2xl leading-none font-bold text-foreground">
                                    {paidLoading
                                        ? '...'
                                        : `Rp ${formatNumber(paidSummary?.paid_total ?? 0)}`}
                                </div>
                            </div>
                        </CardContent>
                    </Card>
            ...
            <Dialog open={poModalOpen} onOpenChange={setPoModalOpen}>
                <DialogContent className="!top-0 !left-0 !h-screen !w-screen !max-w-none !translate-x-0 !translate-y-0 overflow-y-auto !rounded-none">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {selectedRefPo
                                ? `Ref PO: ${selectedRefPo}`
                                : 'Pilih ref PO untuk melihat detail.'}
                        </DialogDescription>
                    </DialogHeader>
                    <div className="max-h-[60vh] overflow-y-auto">
                        ...
                        {!poDetailLoading &&
                            (poHeader || poDetails.length > 0) && (
                                <div className="flex flex-col gap-4">
                                    {poHeader && (
                                        <div className="grid gap-6 rounded-lg border border-sidebar-border/70 p-4 text-sm lg:grid-cols-2">
                                            <div className="space-y-3">
            ...
            <Dialog
                open={unbilledModalOpen}
                onOpenChange={setUnbilledModalOpen}
            >
                <DialogContent className="!top-0 !left-0 !h-screen !w-screen !max-w-none !translate-x-0 !translate-y-0 overflow-hidden !rounded-none">
                    ...
                    <div className="flex h-[calc(100vh-8rem)] flex-col gap-3">
                        <div className="flex flex-wrap gap-3">
                            ...
                            <Select
                                value={
                                    unbilledPageSize === Infinity
                                        ? 'all'
                                        : String(unbilledPageSize)
                                }
                                onValueChange={(val) => {
                                    setUnbilledCurrentPage(1);
                                    setUnbilledPageSize(
                                        ...
                                        >
                                            ...
                                            <TableCell className="text-center">
                                                <div className="flex items-center justify-center gap-2">
                                                    ...
                                                    >
                                                        <Link
                                                            href={`/pembelian/invoice-masuk/${row.no_doc}/edit`}
                                                        >
                            ...
                            filteredUnbilled.length > 0 && (
                                <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-muted-foreground">
                                    <span>
                                        ...
                                        dari {filteredUnbilled.length} data
                                    </span>
                                    <div className="flex items-center gap-2">
                                        <Button
                                            variant="outline"
                                ...
                                <TableBody>
                                    ...
                                    {displayedInvoiceItems.map((item, idx) => (
                                        ...
                                        >
                                            ...
                                            <TableCell className="text-right">
                                                {formatNumber(item.qty_gr ?? 0)}
                                            </TableCell>
                                            <TableCell className="text-right">
                                                Rp{' '}
                                                {formatNumber(item.harga ?? 0)}
                                            </TableCell>
                                            <TableCell className="text-right">
                                                Rp{' '}
                                                {formatNumber(
                            ...
                            filteredInvoiceItems.length > 0 && (
                                <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-muted-foreground">
                                    <span>
                                        ...
                                        dari {filteredInvoiceItems.length} data
                                    </span>
                                    <div className="flex items-center gap-2">
                                        <Button
                                            variant="outline"
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    <ShadcnTableStateRows
                                        ...
                                        error={paidError}
                                        onRetry={() =>
                                            fetchPaid(paidPeriod, paidSearch)
                                        }
                                        isEmpty={
                                            !paidLoading &&
                                            !paidError &&
