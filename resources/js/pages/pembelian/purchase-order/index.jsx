...
const isOutstanding = (item) => Number(item.is_outstanding ?? 0) === 1;

const isPartialStatus = (item) => Number(item.is_partial ?? 0) === 1;

const isRealized = (item) => !isOutstanding(item) && !isPartialStatus(item);
...
export default function PurchaseOrderIndex({
    ...
    const [partialCountState, setPartialCountState] = useState(partialCount);
    const [partialTotalState, setPartialTotalState] = useState(partialTotal);
    ...
    const fetchDataByMaterial = () => {
        ...
        fetch(`/pembelian/purchase-order/data-by-material?${params.toString()}`, {
            ...
            .then((data) => {
                setMatData(Array.isArray(data?.rows) ? data.rows : []);
                setMatTotalRows(Number(data?.total ?? 0));
                setMatHasFetched(true);
            })
            .catch((error) => {
    ...
    const loadPartial = () => {
        ...
        fetch(`/pembelian/purchase-order/partial?${params.toString()}`, {
            ...
            .catch((error) => {
                setPartialError(
                    normalizeApiError(error, 'Gagal memuat data PO sisa GR.'),
                );
            })
            .finally(() => {
        ...
        Swal.fire({
            ...
            fetch(`/pembelian/purchase-order/${encodeURIComponent(noPo)}`, {
                ...
                .then((data) => {
                    ...
                    setPoData((prev) =>
                        prev.filter((item) => item.no_po !== noPo),
                    );
                })
                .catch((error) => {
    ...
    const loadRealized = (customPeriod, force = false) => {
        ...
        if (
            ...
            realizedListPeriod === targetPeriod
        ) {
            return;
        }
    ...
    useEffect(() => {
        setPartialCurrentPage(1);
    }, [partialPageSize, partialSearchTerm]);
    ...
    useEffect(() => {
        if (partialCurrentPage > partialTotalPages) {
            setPartialCurrentPage(partialTotalPages);
        }
    }, [partialCurrentPage, partialTotalPages]);
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <Button
                        type="button"
                        onClick={() =>
                            router.visit('/pembelian/purchase-order/create')
                        }
                    >
                        Tambah PO
                    </Button>
                ...
                <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-4">
                    ...
                    <button
                        ...
                        className="text-left"
                    >
                        <Card className="border-orange-200/50 transition hover:border-primary/60 hover:shadow-md">
                            <CardHeader className="pb-2">
                ...
                {/* ── Tab navigation ───────────────────────────────── */}
                <div className="flex gap-1 rounded-lg border border-sidebar-border/70 bg-muted/40 p-1 w-fit">
                    <button
                        type="button"
                        className={`rounded-md px-4 py-1.5 text-sm font-medium transition-colors ${activeTab === 'po'
                    ...
                    <button
                        type="button"
                        className={`rounded-md px-4 py-1.5 text-sm font-medium transition-colors ${activeTab === 'material'
                                ? 'bg-background text-foreground shadow-sm'
                                : 'text-muted-foreground hover:text-foreground'
                            }`}
                                ...
                                <tbody>
                                    ...
                                    {matData.map((item, idx) => (
                                        <tr
                                            ...
                                            className="border-t border-sidebar-border/70"
                                        >
                                            <td className="w-[1%] px-3 py-2 whitespace-nowrap">{renderValue(item.no_po)}</td>
                                            <td className="w-[1%] px-3 py-2 whitespace-nowrap">{renderValue(item.tgl)}</td>
                                            <td className="w-[1%] px-3 py-2 whitespace-nowrap">{renderValue(item.ref_poin)}</td>
                                            <td className="px-3 py-2">{renderValue(item.for_cus)}</td>
                                            <td className="px-3 py-2">{renderValue(item.nm_vdr)}</td>
                                            <td className="px-3 py-2">{renderValue(item.material)}</td>
                                            <td className="w-[1%] px-3 py-2 text-right whitespace-nowrap">
                                                {renderValue(item.qty)}{item.unit ? ` ${item.unit}` : ''}
                                            </td>
                                            <td className="w-[1%] px-3 py-2 text-right whitespace-nowrap">{formatRupiah(item.price)}</td>
                                            <td className="w-[1%] px-3 py-2 text-right whitespace-nowrap">{formatRupiah(item.total_price)}</td>
                                            <td className="w-[1%] px-3 py-2 text-right whitespace-nowrap">{renderValue(item.gr_mat)}</td>
                                            <td className="w-[1%] px-3 py-2 text-right whitespace-nowrap">{renderValue(item.ir_mat)}</td>
                                            <td className="w-[1%] px-3 py-2 whitespace-nowrap">
                                                <div className="flex items-center justify-end gap-2">
                                                    <ActionIconButton
                                                        label="Detail"
                                                    ...
                                                    <ActionIconButton label="Cetak" asChild>
                                                        <a
                                                            href={`/pembelian/purchase-order/${encodeURIComponent(item.no_po)}/print`}
                                                            target="_blank"
                                                            rel="noreferrer"
                                                        >
                                                            <Printer className="size-4" />
                            ...
                            return matPageSize !== Infinity && matTotalRows > 0 ? (
                                <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-muted-foreground">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm text-muted-foreground">
                                            Halaman {matPage} dari {matTotalPages}
                                        </span>
                                        <Button
                                            variant="outline" size="sm"
                                            onClick={() => setMatPage((p) => Math.min(matTotalPages, p + 1))}
                    ...
                    >
                        <DialogHeader>
                            ...
                            <DialogDescription id="po-realized-desc">
                                Daftar PO terealisasi sesuai periode yang
                                dipilih.
                            </DialogDescription>
                        </DialogHeader>
                        ...
                        <div className="overflow-x-auto rounded-xl border border-sidebar-border/70">
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    ...
                                    />
                                    {displayedRealizedPurchaseOrders.map(
                                        (item) => (
                                            <tr
                ...
                <Dialog
                    ...
                    onOpenChange={(open) => {
                        setIsPartialModalOpen(open);
                        if (open) {
                            loadPartial();
                        } else {
                            setPartialSearchTerm('');
                            setPartialPageSize(5);
                            setPartialCurrentPage(1);
                        }
                    }}
                >
                    ...
                    >
                        <DialogHeader>
                            <DialogTitle>PO Sisa GR</DialogTitle>
                                ...
                                <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-muted-foreground">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <Button
                                            ...
                                            size="sm"
                                            onClick={() =>
                                                setPartialCurrentPage((page) =>
                                                    Math.max(1, page - 1),
                                                )
                                            }
                                            disabled={partialCurrentPage === 1}
                                        >
                                            Sebelumnya
                                        </Button>
                                        <span className="text-sm text-muted-foreground">
                                            Halaman {partialCurrentPage} dari{' '}
                                            {partialTotalPages}
                                        </span>
                                        <Button
                                            variant="outline"
